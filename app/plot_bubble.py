...
def plot_ready(df_prob, df_drug):
    ...
    prob=pd.DataFrame(df_prob['Prob']*100).reset_index()
...
def plot_bubble(df_prob, df_drug,cell_name):
    ...
    trace2 = go.Scatter(x=df_Bubble['xval'],y=df_Bubble['yval'],text=df_Bubble['Label'],mode='markers',
                    marker=dict(size=df_Bubble['Prob'],
                                color=df_Bubble['Prob'],
                                showscale=True)
                    )

    fig = tools.make_subplots(rows=1, cols=2, subplot_titles=('barplot', 'bubble'),vertical_spacing=1)
    fig.append_trace(trace1, 1, 1)
    fig.append_trace(trace2, 1, 2)

    fig['layout']['xaxis1'].update(title='Predicted synergy probability')
    ...
    fig['layout']['yaxis2'].update(title='Drug B',
                                   ...
                                   gridcolor='rgb(255, 255, 255)',
                                   showticklabels=False)
    fig['layout'].update(title='Predicted probability of drug synergy for cell line '+cell_name,
                         titlefont=dict(size=20),
                         showlegend=False,
                         height=600,
                         width=1200,
